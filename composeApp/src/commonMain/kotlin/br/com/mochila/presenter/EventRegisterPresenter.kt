...
class EventRegisterPresenter(private val view: EventRegisterView) {
    ...
    fun saveEvent(userId: Int, event: Event, isEditing: Boolean) {
        ...
        val success = if (isEditing) EventRepository.update(userId, toSave)
        else EventRepository.insert(userId, toSave)
        if (success) {
